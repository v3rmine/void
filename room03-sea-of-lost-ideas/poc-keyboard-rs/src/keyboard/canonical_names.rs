...
lazy_static! {
    ...
    static ref SIDED_MODIFIERS: Vec<&'static str> = vec!["ctrl", "alt", "shift", "windows"];
}

#[cfg(not(target_os = "macos"))]
pub static ALL_MODIFIERS: &'static [&'static str] = &[
    "ctrl", "alt", "alt gr", "shift", "windows", // general
    "left ctrl", "left alt", "left alt gr", "left shift", "left windows", // left
    "right ctrl", "right alt", "right alt gr", "right shift", "right windows", // right
];
    
#[cfg(target_os = "macos")]
pub static ALL_MODIFIERS: &'static [&'static str] = &["ctrl", "alt", "shift", "windows"];

pub fn normalize_name(name: &str) -> Result<&str> {
    let mut result = String::new();
    if name.len() > 1 {
        result = name.to_lowercase();
    }
    if name != "_" && name.contains('_') {
        result = name.replace("_"," ");
    }

    Ok(CANONICAL_NAMES.get(result.as_str()).unwrap_or_else(|_| KeyNotFound)?)
}
...
impl std::fmt::Display for KeyNotFound {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "Key not found in internal dictionary")
    }
}