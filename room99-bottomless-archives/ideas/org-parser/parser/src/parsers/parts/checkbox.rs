use crate::{
    Checkbox, PResult, ParseTextToAst, CHECKBOX_CHECKED_SYMBOL, CHECKBOX_SURROUNDING_SYMBOL_END,
...
impl<'input> ParseTextToAst<'input> for Checkbox {
...
mod tests {
    ...
    use crate::{Checkbox, ParseTextToAst};

    #[assay]
    fn parse_checkbox() {
        let (_, checked_checkbox) = Checkbox::parse_text_to_ast("[x]".into())?;
        let (_, empty_checkbox) = Checkbox::parse_text_to_ast("[ ]".into())?;
