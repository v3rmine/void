...
use crate::{
    Link, PResult, Parser, LINK_DESCRIPTION_SURROUNDING_SYMBOL_START,
...
impl<'input> Parser<'input> for Link<'input> {
...
mod tests {
    ...
    #[assay]
    fn parse_web_link() {
        let (_, parsed) = Link::parse_text("[[https://example.org/]]".into())?;
