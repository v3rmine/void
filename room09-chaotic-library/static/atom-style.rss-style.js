      ...
      if (itemDesc) {
        try {
          if (itemDesc.indexOf("<") !== -1 && itemDesc.indexOf(">") !== -1) {
            // Edit use a DOMParser
            const parser = new DOMParser();
            const doc = parser.parseFromString(itemDesc, "text/html");
            const descContainer = document.createElementNS(NS, "div");
            // Append all child nodes from the parsed document's body to the descContainer
            while (doc.body.firstChild) {
              descContainer.appendChild(doc.body.firstChild);
            }
